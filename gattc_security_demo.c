...
#define REMOTE_SERVICE1_UUID   0x00FF
#define REMOTE_SERVICE2_UUID   0x00EE
#define REMOTE_NOTIFY_UUID     0x2A37
static esp_gattc_char_elem_t *char_elem_result   = NULL;
static esp_gattc_char_elem_t *char1_res=NULL;
static esp_gattc_char_elem_t *char_elem_result1   = NULL;
static esp_gattc_char_elem_t *char2_res=NULL;
...
static void  writechar1(void);
static void  writechar2(void);

static esp_bt_uuid_t remote_filter_service_uuid = {
    .len = ESP_UUID_LEN_16,
    .uuid = {.uuid16 = REMOTE_SERVICE1_UUID,},
};

static esp_bt_uuid_t remote_filter_service2_uuid = {
    .len = ESP_UUID_LEN_16,
    .uuid = {.uuid16 = REMOTE_SERVICE2_UUID,},
};

static bool connect = false;
static bool get_service = false;
static bool get_service2 = false;
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CFG_MTU_EVT:
        ...
        esp_ble_gattc_search_service(gattc_if, param->cfg_mtu.conn_id, &remote_filter_service2_uuid);
        break;
    case ESP_GATTC_SEARCH_RES_EVT: {
        ESP_LOGI(GATTC_TAG, "SEARCH RES: conn_id = %x is primary service %d", p_data->search_res.conn_id, p_data->search_res.is_primary);
        ESP_LOGI(GATTC_TAG, "start handle %d end handle %d current handle value %d", p_data->search_res.start_handle, p_data->search_res.end_handle, p_data->search_res.srvc_id.inst_id);
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == REMOTE_SERVICE1_UUID) {
            ESP_LOGI(GATTC_TAG, "UUID16: %x", p_data->search_res.srvc_id.uuid.uuid.uuid16);
            get_service = true;
            gl_profile_tab[PROFILE_A_APP_ID].service_start_handle = p_data->search_res.start_handle;
            gl_profile_tab[PROFILE_A_APP_ID].service_end_handle = p_data->search_res.end_handle;
        }
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == REMOTE_SERVICE2_UUID) {
            ESP_LOGI(GATTC_TAG, "UUID16: %x", p_data->search_res.srvc_id.uuid.uuid.uuid16);
            get_service2 = true;
            gl_profile_tab[PROFILE_A_APP_ID].service_start_handle = p_data->search_res.start_handle;
            gl_profile_tab[PROFILE_A_APP_ID].service_end_handle = p_data->search_res.end_handle;
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if (get_service){
            ...
            if (count > 0){
                ...
                char1_res = (esp_gattc_char_elem_t *)malloc(sizeof(esp_gattc_char_elem_t) * count); 
                if (!char_elem_result){
                    ...
                    ESP_LOGE(GATTC_TAG, "gattc no mem");}
                        ...
                        memcpy(char_elem_result,char1_res,sizeof(esp_gattc_char_elem_t));}

                        flag=true; 
                 /*  
                    const char *char_string = "Hello, BLE2!";  // Replace this with your actual string
                      uint16_t char_length = strlen(char_string);

                    // Allocate memory for the byte array and copy the string
                      uint8_t *char_data = (uint8_t *)malloc(char_length);
                      memcpy(char_data, char_string, char_length);

                    // Now, use esp_ble_gattc_write_char to send the data
        ...
        if (get_service2){
            ...
            if (count > 0){
                char_elem_result1 = (esp_gattc_char_elem_t *)malloc(sizeof(esp_gattc_char_elem_t) * count);
                char2_res = (esp_gattc_char_elem_t *)malloc(sizeof(esp_gattc_char_elem_t) * count); 
                if (!char_elem_result1){
                    ESP_LOGE(GATTC_TAG, "gattc no mem");
                }else{
                    ret_status = esp_ble_gattc_get_all_char(gattc_if,
                                                            gl_profile_tab[PROFILE_A_APP_ID].conn_id,
                    ...
                    if (count > 0){
                        ...
                        for (int i = 0; i < count; ++i)
                        {
                            if (char_elem_result1[i].uuid.len == ESP_UUID_LEN_16 && char_elem_result1[i].uuid.uuid.uuid16 == REMOTE_NOTIFY_UUID && (char_elem_result1[i].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY))
                            {
                                gl_profile_tab[PROFILE_A_APP_ID].notify_char_handle = char_elem_result1[i].char_handle;
                                esp_ble_gattc_register_for_notify (gattc_if,
                                                                   gl_profile_tab[PROFILE_A_APP_ID].remote_bda,
                                                                   char_elem_result1[i].char_handle);
                    ...
                    ESP_LOGE(GATTC_TAG, "gattc no mem");
                     ...
                     {
                      memcpy(char_elem_result1,char2_res,sizeof(esp_gattc_char_elem_t));
                     }
                     flag2=true; 
                     /* const char *char_string2 = "Hello, BLE Service 2 Char 1 !";  // Replace this with your actual string
                      uint16_t char_length2 = strlen(char_string2);

                    // Allocate memory for the byte array and copy the string
                      uint8_t *char_data2 = (uint8_t *)malloc(char_length2);
                      memcpy(char_data2, char_string2, char_length2);
                    ...
                    // Now, use esp_ble_gattc_write_char to send the data
                      esp_err_t write_status =esp_ble_gattc_write_char(gattc_if,
                         ...
                         ESP_GATT_AUTH_REQ_NONE);
                   esp_ble_gattc_read_char(gattc_if,gl_profile_tab[PROFILE_A_APP_ID].conn_id,
                          char_elem_result1[0].char_handle,
                         ESP_GATT_AUTH_REQ_NONE);
                           if(write_status!=ESP_OK)
                   {
                    printf("Write failed");
    ...
    case ESP_GATTC_READ_CHAR_EVT:
       ...
       esp_log_buffer_char(GATTC_TAG,p_data->read.value,p_data->read.value_len);   
       break; 
...
void writechar1()
{
 const char *char_string = "Hello, BLE2!";  // Replace this with your actual string
                    ...
                    // Now, use esp_ble_gattc_write_char to send the data
                      esp_err_t write_status =esp_ble_gattc_write_char(gl_profile_tab[PROFILE_A_APP_ID].gattc_if,
                        gl_profile_tab[PROFILE_A_APP_ID].conn_id,
                         char1_res[0].char_handle,
                         char_length,
                         char_data,
                         ESP_GATT_WRITE_TYPE_NO_RSP,
                         ESP_GATT_AUTH_REQ_NONE);
                    esp_ble_gattc_read_char(client_if,client_conn,
                          char2_res[2].char_handle,
                         ESP_GATT_AUTH_REQ_NONE);
                   if(write_status!=ESP_OK)
                   {
                    printf("Write failed");
...
const char *char_string2 = "Hello, BLE Service 2 Char 1 !";  // Replace this with your actual string
                    ...
                    // Now, use esp_ble_gattc_write_char to send the data
                      esp_err_t write_status =esp_ble_gattc_write_char(client_if,
                         client_conn,
                         char2_res[0].char_handle,
                         char_length2,
                         char_data2,
                         ESP_GATT_WRITE_TYPE_NO_RSP,
                         ESP_GATT_AUTH_REQ_NONE);
